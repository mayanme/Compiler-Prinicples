...
(define fold-left
    (lambda (func acc lst)
        (letrec ((helper (lambda (func acc lst)
                        (if (eq? lst '())
                            acc
                            (helper func (func acc (car lst)) (cdr lst))
                            ))
                        ))
            (helper func acc lst)
            )
...
(define fold-right
    (lambda (func acc lst)
        (letrec ((helper (lambda (func acc lst)
                        (if (eq? lst '())
                            acc
                            (func (car lst) (helper func acc (cdr lst)))
                            ))
                        ))
            (helper func acc lst)
            )
...
(define cons*
	(let ((cons cons)
		(null? null?))
		(lambda (x . y)
			(letrec ((fold-right-cons 
						(lambda (lst) 
							(if (null? (cdr lst))
							(car lst)
							(cons (car lst) (fold-right-cons (cdr lst))))))
					(args-list (cons x y)))
			(fold-right-cons args-list)))))
